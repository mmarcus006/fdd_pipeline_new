...
def cleanup_all_browsers():
    ...
    try:
        ...
        if system == "windows":
            ...
            subprocess.run(
                ["pkill", "-f", "chromium"], capture_output=True, check=False
            )
...
def get_franchise_names():
    ...
    table_html_string = page.evaluate(
        "document.getElementById('ctl00_contentPlaceholder_grdActiveFilings').outerHTML"
    )
...
def process_franchise(franchise_name):
    ...
    ) or page.query_selector('input[type="submit"], button[type="submit"]')
    if search_button:
        search_button.click()
    ...
    if details_link:
        ...
        franchisor_name_address_match = re.search(
            r'group "Franchisor Name and Address" \[ref=e\d+\]:\n(.*?)(?=group "Filings for this Registration")',
            details_snapshot_output,
            re.DOTALL,
        )
        if franchisor_name_address_match:
            ...
            filing_number_match = re.search(
                r'Filing Number.*?generic \[ref=e\d+\]: "(\d+)"', section_content
            )
            franchisor_info["Filing Number"] = (
                filing_number_match.group(1) if filing_number_match else ""
            )

            filing_status_match = re.search(
                r"Filing Status.*?generic \[ref=e\d+\]: (\w+)", section_content
            )
            franchisor_info["Filing Status"] = (
                filing_status_match.group(1) if filing_status_match else ""
            )

            legal_name_match = re.search(
                r"Franchise Legal Name.*?generic \[ref=e\d+\]: (.*?)\n", section_content
            )
            franchisor_info["Franchise Legal Name"] = (
                legal_name_match.group(1).strip() if legal_name_match else ""
            )

            trade_name_match = re.search(
                r"Franchise Trade Name \(DBA\).*?generic \[ref=e\d+\]: (.*?)\n",
                section_content,
            )
            franchisor_info["Franchise Trade Name (DBA)"] = (
                trade_name_match.group(1).strip() if trade_name_match else ""
            )

            # Extracting multi-line address
            address_lines = re.findall(
                r"Franchise Business Address.*?generic \[ref=e\d+\]: (.*?)\n|^\s+- cell \[ref=e\d+\]:\n\s+- cell \[ref=e\d+\]:\n\s+- generic \[ref=e\d+\]: (.*?)\n|^\s+- cell \[ref=e\d+\]:\n\s+- cell \[ref=e\d+\]:\n\s+- generic \[ref=e\d+\]: (.*?)\n",
                section_content,
                re.MULTILINE,
            )
        ...
        filings_section_match = re.search(
            r'group "Filings for this Registration" \[ref=e\d+\]:\n(.*?)(?=group "States Application Filed")',
            details_snapshot_output,
            re.DOTALL,
        )
        if filings_section_match:
            ...
            filings_info["Effective"] = (
                effective_match.group(1).strip() if effective_match else ""
            )

        # Extract States Application Filed
        states_section_match = re.search(
            r'group "States Application Filed" \[ref=e\d+\]:\n(.*?)(?=group "Contact Person")',
            details_snapshot_output,
            re.DOTALL,
        )
        if states_section_match:
            ...
            states_list_match = re.search(
                r"States Filed.*?generic \[ref=e\d+\]:\n(.*)",
                section_content,
                re.DOTALL,
            )
        ...
        combined_data = {
            ...
            "Franchise Trade Name (DBA)": franchisor_info.get(
                "Franchise Trade Name (DBA)", ""
            ),
            "Franchise Business Address": franchisor_info.get(
        ...
        download_button = page.query_selector("#e199") or page.query_selector(
        ...
        clear_button = page.query_selector("#e43") or page.query_selector(
            'input[type="reset"], button:has-text("Clear")'
        )
        if clear_button:
            clear_button.click()
    else:
        print(
            f"No 'Registered' details link found for {franchise_name}. Skipping to next franchise."
        )
