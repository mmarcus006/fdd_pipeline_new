...
class MonitoringService:
    ...
    def _get_pending_documents(self) -> int:
        ...
        try:
            result = self.db.query_builder("fdds").select("count").eq(
    ...
    def _get_failed_extractions(self) -> int:
        ...
        try:
            ...
            result = self.db.query_builder("fdd_sections").select("count").eq(
    ...
    def _calculate_processing_rate(self) -> float:
        ...
        try:
            ...
            ).gte("processed_at", cutoff_time.isoformat()).execute()
    ...
    def _calculate_error_rate(self) -> float:
        ...
        try:
            ...
            # Get total attempts
            total_result = self.db.query_builder("fdd_sections").select("count").gte(
            ...
            # Get failed attempts
            failed_result = self.db.query_builder("fdd_sections").select("count").eq(
                "extraction_status", "failed"
            ).gte("created_at", cutoff_time.isoformat()).execute()
