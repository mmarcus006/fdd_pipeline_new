...
class ConfigValidator:
        ...
        if env_file.exists():
            ...
            if env_template.exists():
                self.warnings.append(
                    "⚠️  No .env file found - copy .env.template and configure"
                )
            else:
    ...
    def validate_database(self, settings: Settings):
        ...
        if settings.supabase_url and settings.supabase_anon_key:
            ...
            if settings.supabase_service_key:
                ...
                self.warnings.append(
                    "⚠️  No Supabase service key - some operations may fail"
                )
        else:
    ...
    def validate_google_drive(self, settings: Settings):
        ...
        if settings.gdrive_creds_json:
            ...
            if creds_path.exists():
                self.info.append(
                    f"✅ Google Drive credentials file exists: {creds_path}"
                )
            else:
                self.errors.append(
    ...
    def validate_llm_apis(self, settings: Settings):
        ...
        if settings.ollama_base_url:
            self.info.append(
                f"✅ Ollama base URL configured: {settings.ollama_base_url}"
            )
    ...
    def validate_mineru(self, settings: Settings):
        ...
        if settings.mineru_mode == "local":
            ...
            if model_path.exists():
                ...
                model_files = list(model_path.glob("**/*.pth")) + list(
                    model_path.glob("**/*.onnx")
                )
                if model_files:
                    self.info.append(
                        f"✅ MinerU models found: {len(model_files)} files"
                    )
                else:
                    self.warnings.append(
                        "⚠️  No model files found - run: magic-pdf model-download"
                    )
            else:
                self.warnings.append(
                    f"⚠️  MinerU model path does not exist: {model_path}"
                )
            ...
            if settings.mineru_device == "cuda":
                try:
                    ...
                    if torch.cuda.is_available():
                        ...
                        self.warnings.append(
                            "⚠️  CUDA not available - will fall back to CPU"
                        )
                except ImportError:
                    self.warnings.append(
                        "⚠️  PyTorch not installed - GPU detection unavailable"
                    )
    ...
    def validate_prefect(self, settings: Settings):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.warnings.append(
                    "⚠️  Prefect server not responding - start with: prefect server start"
                )
        except Exception:
    ...
    def print_results(self):
        ...
        print("\n" + "-" * 60)
        print(
            f"Summary: {len(self.info)} OK, {len(self.warnings)} warnings, {len(self.errors)} errors"
        )
