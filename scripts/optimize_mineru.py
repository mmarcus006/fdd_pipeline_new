...
class MinerUOptimizer:
    ...
    def apply_recommendations(self, recommendations: dict):
        ...
        config["formula-config"]["batch_size"] = recommendations.get("formula_batch_size", 16)
        ...
        if recommendations["device"] == "cuda":
            config["optimization"]["gpu_memory_fraction"] = recommendations.get("gpu_memory_fraction", 0.8)
        
        config["optimization"]["num_workers"] = recommendations.get("num_workers", 4)
        config["optimization"]["prefetch_factor"] = recommendations.get("prefetch_factor", 2)
        ...
        if recommendations["device"] == "cpu":
            config["fallback"]["cpu_num_threads"] = recommendations.get("cpu_num_threads", 8)
    ...
    def update_environment(self, recommendations: dict):
        ...
        # CPU settings
        if recommendations["device"] == "cpu":
            env_updates.append(f"export OMP_NUM_THREADS={recommendations.get('cpu_num_threads', 8)}")
...
"""
            ...
            result = subprocess.run(
                ...
                text=True,
                env={**os.environ, **dict(
                    MINERU_DEVICE=self.system_info.get("device", "cpu")
                )}
            )
            ...
            print("pip install magic-pdf[full] --extra-index-url https://wheels.myhloli.com")
    ...
    def print_report(self, recommendations: dict):
        ...
        if recommendations["device"] == "cuda":
            ...
            print(f"  GPU Memory: {recommendations.get('gpu_memory_fraction', 0.8) * 100:.0f}%")
        else:
...
def main():
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Apply recommendations to configuration"
    )
    parser.add_argument(
        "--benchmark",
        action="store_true",
