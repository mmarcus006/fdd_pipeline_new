    ...
    try:
        ...
        async with create_scraper(MinnesotaScraper, headless=False, timeout=60000) as scraper:
            ...
            for i, doc in enumerate(documents[:5]):
                ...
                print(f"   Document ID: {doc.additional_metadata.get('document_id', 'N/A')}")
            ...
            print(f"✅ Test completed successfully! Found {len(documents)} total documents")
...
async def test_document_download():
    ...
    try:
        async with create_scraper(MinnesotaScraper, headless=True, timeout=60000) as scraper:
