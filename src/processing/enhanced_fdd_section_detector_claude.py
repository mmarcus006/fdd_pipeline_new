...
class EnhancedFDDSectionDetector:
    ...
    def _prepare_reference_vectors(self):
        ...
        logger.info(
            f"Prepared reference vectors for {len(all_headers)} section headers"
        )

    def detect_sections_from_mineru_json(
        self, mineru_json_path: str, total_pages: Optional[int] = None
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            for c in candidates:
                ...
                candidate_summary[c.item_no].append(
                    (c.page_number, c.confidence, c.detection_method)
                )

            logger.debug("=== CANDIDATE SUMMARY ===")
            for item_no in sorted(candidate_summary.keys()):
                logger.debug(
                    f"Item {item_no:2d}: {len(candidate_summary[item_no])} candidates"
                )
                for page, conf, method in sorted(candidate_summary[item_no])[
                    :3
                ]:  # Top 3
                    logger.debug(
                        f"  Page {page:3d} | Conf: {conf:.2f} | Method: {method}"
                    )

        # Apply validation rules and resolve conflicts
        validated_sections = self._validate_and_resolve_candidates(
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            for section in validated_sections:
                logger.debug(
                    f"Item {section.item_no:2d}: Page {section.page_number:3d} | "
        ...
        section_boundaries = self._create_section_boundaries(
            validated_sections, total_pages
        )
    ...
    def _extract_section_candidates(
        ...
        for page_data in mineru_data["pdf_info"]:
            ...
            if page_number <= appendix_threshold:
                cosine_candidates = self._detect_from_cosine_similarity(
                    page_data, page_idx
                )
    ...
    def _filter_candidates(
        self, candidates: List[FDDSectionCandidate], total_pages: int
    ) -> List[FDDSectionCandidate]:
        ...
        for candidate in candidates:
            ...
            if key not in seen_combinations:
                ...
                for i, existing in enumerate(filtered):
                    if (
                        existing.item_no == candidate.item_no
                ...
                if (
                    ...
                    and candidate.confidence > filtered[existing_idx].confidence
                ):
    ...
    def _detect_from_title_elements(
        self, page_data: Dict[str, Any], page_idx: int
    ) -> List[FDDSectionCandidate]:
        ...
        for para_block in page_data.get("para_blocks", []):
            ...
            logger.debug(
                f"  Found title element: '{text_content[:100]}...' on page {page_number}"
            )
            ...
            if item_match:
                ...
                logger.debug(
                    f"    -> MATCHED Item {item_no}: '{item_name}' (confidence: 0.95)"
                )
        ...
        logger.debug(
            f"Page {page_number}: Found {title_count} title elements, {len(candidates)} title candidates"
        )
        return candidates
        ...
        for para_block in page_data.get("para_blocks", []):
            ...
            for item_no, item_text in item_matches:
                ...
                if 0 <= item_no <= 24:
                    candidate = FDDSectionCandidate(
                        item_no=item_no,
                        item_name=self.STANDARD_FDD_SECTIONS.get(
                            item_no, f"Item {item_no}"
                        ),
                        page_idx=page_idx,
                        page_number=page_idx + 1,
    ...
    def _detect_from_fuzzy_matching(
        ...
        for para_block in page_data.get("para_blocks", []):
            ...
            if not self._looks_like_section_header(text_content):
                logger.debug(
                    f"  Text doesn't look like header: '{text_content[:50]}...'"
                )
                continue
            ...
            if best_match and best_match[1] >= self.min_fuzzy_score:
                ...
                logger.debug(
                    f"    -> Fuzzy match: Item {item_no} (score: {score}) matched '{matched_header}'"
                )

                # Additional validation: check if the match makes sense
                if not self._validate_section_match(item_no, text_content):
                    logger.debug(
                        f"    -> REJECTED: Failed section validation for Item {item_no}"
                    )
                    continue

                fuzzy_matches += 1
                logger.debug(
                    f"    -> ACCEPTED: Item {item_no} fuzzy match (confidence: {score/100.0:.2f})"
                )
                ...
                if best_match:
                    logger.debug(
                        f"    -> Score too low: {best_match[1]} < {self.min_fuzzy_score}"
                    )
                else:
                    logger.debug(f"    -> No fuzzy match found")

        logger.debug(
            f"Page {page_number}: Processed {blocks_processed} blocks, {header_like_count} header-like, {fuzzy_matches} fuzzy matches"
        )
        return candidates
    ...
    def _detect_from_cosine_similarity(
        ...
        for para_block in page_data.get("para_blocks", []):
            ...
            try:
                ...
                similarities = cosine_similarity(text_vector, self._reference_vectors)[
                    0
                ]
                ...
                if max_similarity >= self.confidence_threshold:
                    ...
                    item_name = self.STANDARD_FDD_SECTIONS.get(
                        item_no_int, f"Item {item_no_int}"
                    )
                    candidate = FDDSectionCandidate(
                        item_no=item_no_int,
                        item_name=item_name,
    ...
    def _looks_like_section_header(self, text: str) -> bool:
        ...
        if len(text) > 150:
            logger.debug(
                f"      Header check FAILED: Too long ({len(text)} chars) - '{text[:50]}...'"
            )
            return False
        ...
        if text_upper == text or text.istitle():
            logger.debug(
                f"      Header check PASSED: All caps or title case - '{text[:50]}...'"
            )
            return True
        ...
        if "ITEM " in text_upper and any(char.isdigit() for char in text):
            logger.debug(
                f"      Header check PASSED: Contains 'Item X' pattern - '{text[:50]}...'"
            )
            return True
        ...
        if matched_patterns and len(text) < 100:
            logger.debug(
                f"      Header check PASSED: Contains pattern '{matched_patterns[0]}' - '{text[:50]}...'"
            )
            return True
    ...
    def _validate_section_match(self, item_no: int, text: str) -> bool:
        ...
        validation_rules = {
            8: [
                ...
                "products",
                "services",
            ],  # Item 8 should be about restrictions
            5: [
                "initial",
                "fee",
                "franchise fee",
            ],  # Item 5 should mention initial/franchise fees
            6: [
                "other",
                "fee",
                "ongoing",
                "royalty",
            ],  # Item 6 should mention other fees
            7: [
                "investment",
                "initial",
                "estimated",
            ],  # Item 7 should mention investment
            21: [
                "financial",
                "statement",
                "audit",
            ],  # Item 21 should mention financial statements
            19: [
                "financial",
                "performance",
                "representation",
        ...
        if item_no in validation_rules:
            ...
            if not found_keywords:
                logger.debug(
                    f"        Validation FAILED: Item {item_no} missing required keywords {required_keywords}"
                )
                return False
            else:
                logger.debug(
                    f"        Validation check 1 PASSED: Item {item_no} found keywords {found_keywords}"
                )
        ...
        wrong_matches = {
            8: [
                ...
                "audited",
                "balance sheet",
            ],  # Item 8 is NOT about financials
            5: [
                "adjusted gross revenue",
                "note 2:",
                "royalty fee",
            ],  # Item 5 is NOT about ongoing fees
        }
        ...
        if item_no in wrong_matches:
            ...
            if found_wrong:
                logger.debug(
                    f"        Validation FAILED: Item {item_no} contains wrong keywords {found_wrong}"
                )
                return False
        ...
        for item_no in range(25):  # Items 0-24
            ...
            if item_no in by_item:
                ...
                for i, candidate in enumerate(by_item[item_no][:3]):  # Show top 3
                    valid_marker = (
                        "✓" if candidate.page_number >= current_min_page else "✗"
                    )
                    logger.debug(
                        f"    {i+1}. {valid_marker} Page {candidate.page_number} | "
            ...
            if item_no == 0:
                ...
                logger.debug(
                    f"  Updated min_page: {prev_min_page} -> {current_min_page}"
                )

            logger.debug(
                f"  FINAL: Item {item_no} assigned to page {section.page_number} "
    ...
    def _has_exact_item_pattern(self, text: str, item_no: int) -> bool:
        ...
        if item_no == 24:
            appendix_patterns = [r"^APPENDIX", r"^EXHIBIT", r"^ATTACHMENT"]
    ...
    def _find_sequential_section(
        ...
        logger.debug(
            f"  Evaluating {len(all_candidates)} candidates for Item {item_no} "
        ...
        if phase1_candidates:
            ...
            logger.info(
                f"  PHASE 1 MATCH: Item {item_no} found title with 'Item {item_no}' "
        ...
        phase2_candidates = [
            c
            for c in all_candidates
            if c.detection_method == "fuzzy" and min_page <= c.page_number < max_page
        ]
        ...
        if phase2_candidates:
            ...
            logger.info(
                f"  PHASE 2 MATCH: Item {item_no} fuzzy match "
                f"on page {best.page_number} (conf: {best.confidence:.2f}, "
        ...
        phase3_candidates = [
            c
            for c in all_candidates
            if c.detection_method == "pattern" and min_page <= c.page_number < max_page
        ]

        if phase3_candidates:
            best = max(phase3_candidates, key=lambda x: (x.confidence, -x.page_number))
            logger.info(
                f"  PHASE 3 MATCH: Item {item_no} pattern match "
                f"on page {best.page_number} (conf: {best.confidence:.2f})"
            )
            return best
        ...
        phase4_candidates = [
            c
            for c in all_candidates
            if c.detection_method == "cosine" and min_page <= c.page_number < max_page
        ]

        if phase4_candidates:
            best = max(phase4_candidates, key=lambda x: (x.confidence, -x.page_number))
            logger.info(
                f"  PHASE 4 MATCH: Item {item_no} cosine match "
                f"on page {best.page_number} (conf: {best.confidence:.2f})"
            )
            return best
        ...
        logger.warning(
            f"  No valid candidates for Item {item_no} in page range {min_page}-{max_page}"
        )
        return self._create_interpolated_section(item_no, min_page, total_pages)

    def _create_interpolated_section(
        self, item_no: int, min_page: int, total_pages: Optional[int]
    ) -> FDDSectionCandidate:
        ...
        logger.info(
            f"Interpolating Item {item_no} at page {interpolated_page} (min_page: {min_page}, estimated: {estimated_page})"
        )
    ...
    def _create_section_boundaries(
        ...
        for i, candidate in enumerate(candidates):
            ...
            boundary = SectionBoundary(
                ...
                end_page=end_page,
                confidence=min(
                    1.0, float(candidate.confidence)
                ),  # Ensure confidence <= 1.0
            )
...
def main():
    ...
    if Path(sample_json_path).exists():
        sections = detector.detect_sections_from_mineru_json(
            sample_json_path, total_pages=75
        )
