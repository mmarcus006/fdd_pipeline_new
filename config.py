...
class Settings(BaseSettings):
    ...
    gdrive_creds_json: str = Field(
        ..., description="Path to Google service account JSON"
    )
    gdrive_folder_id: str = Field(
        ..., description="Google Drive folder ID for FDD storage"
    )

    # MinerU Configuration (API and Local)
    mineru_mode: str = Field(
        default="local", description="MinerU processing mode: 'api' or 'local'"
    )
    mineru_api_key: Optional[str] = Field(
        default=None, description="MinerU API key (required for API mode)"
    )
    mineru_base_url: str = Field(
        default="https://api.mineru.com/v1", description="MinerU API base URL"
    )
    ...
    mineru_batch_size: int = Field(
        default=2, description="Batch size for MinerU processing (local mode)"
    )
    mineru_rate_limit: int = Field(
        default=10, description="Maximum concurrent MinerU API requests"
    )
    ...
    alert_recipients: str = Field(
        default="", description="Comma-separated email addresses for alerts"
    )
    ...
    prefect_api_key: Optional[str] = Field(
        default=None, description="Prefect API key (for cloud)"
    )

    # Processing Configuration
    max_concurrent_extractions: int = Field(
        default=5, description="Maximum concurrent LLM extractions"
    )
    extraction_timeout_seconds: int = Field(
        default=300, description="Timeout for LLM extractions"
    )
