...
async def download_and_save_document(
    ...
    try:
        ...
        if skip_existing and filepath.exists():
            logger.info(
                "file_already_exists", franchise=franchise_name, filepath=str(filepath)
            )
            return filepath
        ...
        if success:
            logger.info(
                "document_saved", franchise=franchise_name, filepath=str(filepath)
            )
            return filepath
...
def export_documents_to_csv(
    documents: List[DocumentMetadata], filepath: Path, state_code: str
) -> bool:
    ...
    try:
        ...
        if rows:
            ...
            logger.info(
                "csv_export_completed", filepath=str(filepath), row_count=len(rows)
            )
            return True
        ...
        logger.error("csv_export_failed", filepath=str(filepath), error=str(e))
        return False
