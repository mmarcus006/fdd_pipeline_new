...
from tasks.web_scraping import BaseScraper, DocumentMetadata, ExtractionError, NavigationError
...
class WisconsinScraper(BaseScraper):
    ...
    async def discover_documents(self) -> List[DocumentMetadata]:
        ...
        try:
            ...
            raise ExtractionError(f"Wisconsin document discovery failed: {e}")
    ...
    async def extract_document_metadata(self, document_url: str) -> DocumentMetadata:
        ...
        try:
            ...
            if not details_url:
                self.logger.warning("no_registered_details_found", franchise=franchise_name)
                return None
            ...
            raise ExtractionError(f"Failed to extract detailed metadata: {e}")
    ...
    async def _extract_franchise_names_from_table(self) -> List[str]:
        ...
        try:
            ...
            if not table_html:
                raise ExtractionError("Failed to retrieve table HTML")
            
            # Parse franchise names from table rows
            franchise_names = []
            ...
            for i, row in enumerate(rows):
                ...
                if cells:
                    ...
                    name = cells[0].strip()
                    # Remove HTML tags first (but preserve content that looks like HTML entities)
                    name = re.sub(r'<(?!/?[a-zA-Z])[^>]*>', '', name)  # Remove HTML tags but not < > in text
            ...
            self.logger.info(
                "franchise_names_extracted_from_table",
                count=len(franchise_names)
            )
            ...
            raise ExtractionError(f"Failed to extract franchise names from table: {e}")
    
    async def _search_franchise_basic(self, franchise_name: str) -> Optional[DocumentMetadata]:
        ...
        """
        try:
            self.logger.debug("performing_basic_franchise_search", franchise=franchise_name)
            ...
            if not has_registered:
                self.logger.debug("no_registered_franchise_found", franchise=franchise_name)
                return None
            ...
            self.logger.error(
                "basic_franchise_search_failed",
                franchise=franchise_name,
                error=str(e)
            )
            raise ExtractionError(f"Basic franchise search failed for {franchise_name}: {e}")
    ...
    async def _perform_franchise_search(self, franchise_name: str) -> None:
        ...
        Raises:
            ExtractionError: If search operation fails
        """
        try:
            ...
            if not search_input:
                raise ExtractionError("Search input field not found")
            ...
            for selector in search_button_selectors:
                try:
                    search_button = await self.page.wait_for_selector(selector, timeout=5000)
                    if search_button:
                        break
                except:
                    continue
            ...
            if not search_button:
                raise ExtractionError("Search button not found")
            ...
            raise ExtractionError(f"Franchise search operation failed: {e}")
            ...
            for link in details_links:
                ...
                try:
                    row_text = await self.page.evaluate(
                        '(element) => element.closest("tr").textContent',
                        link
                    )
    ...
    async def _extract_detailed_filing_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if franchisor_match:
                ...
                filing_number_match = re.search(r'Filing Number.*?generic.*?: "(\d+)"', section_content)
                if filing_number_match:
                    franchisor_info['filing_number'] = filing_number_match.group(1)
                
                # Extract filing status
                status_match = re.search(r'Filing Status.*?generic.*?: (\w+)', section_content)
                if status_match:
                    franchisor_info['filing_status'] = status_match.group(1)
                
                # Extract legal name
                legal_name_match = re.search(r'Franchise Legal Name.*?generic.*?: (.*?)\n', section_content)
                if legal_name_match:
                    franchisor_info['legal_name'] = legal_name_match.group(1).strip()
                
                # Extract trade name
                trade_name_match = re.search(r'Franchise Trade Name \(DBA\).*?generic.*?: (.*?)\n', section_content)
                if trade_name_match:
                ...
                address_match = re.search(r'Franchise Business Address.*?generic.*?: (.*?)\n', section_content)
                full_address = []
                if address_match:
            ...
            if states_match:
                ...
                states_list_match = re.search(r'States Filed.*?generic.*?:\n(.*)', section_content, re.DOTALL)
            ...
            download_url = None
            try:
                download_button = await self.page.query_selector('button:has-text("Download"), input[value*="Download"]')
                if download_button:
