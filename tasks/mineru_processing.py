...
class MinerUProcessor:
                    ...
                    for cookie in cookies:
                        if cookie['name'] == 'uaa-token':
                            ...
                            if self._verify_auth(auth_data):
                                ...
                                self.logger.info("Successfully loaded saved authentication")
                                return True
                            else:
                                self.logger.warning("Saved auth expired, need to login again")
    ...
    def _verify_auth(self, auth_data: Dict) -> bool:
        ...
        try:
            with sync_playwright() as p:
                ...
                try:
                    page.wait_for_selector("button:has-text('URL Upload')", timeout=3000)
                    browser.close()
                    return True
                except:
                    browser.close()
    ...
    async def _wait_for_completion(self, task_id: str, max_wait: int = 300) -> bool:
        ...
        while time.time() - start < max_wait:
            ...
            response = await asyncio.to_thread(
                self.session.get,
                f"{self.api_tasks}?page_no=1&page_size=20&type="
            )
            data = response.json()
    ...
    async def _get_results(self, task_id: str) -> Dict[str, Any]:
        ...
        response = await asyncio.to_thread(
            self.session.get,
            self.api_detail.format(task_id=task_id)
        )
    ...
    try:
        ...
        sections = [
            FDDSection(
                ...
                page_count=1,
                mineru_json_path=mineru_results["drive_files"].get("json", {}).get("drive_path"),
