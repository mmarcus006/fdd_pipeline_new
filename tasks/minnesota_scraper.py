...
class MinnesotaScraper(BaseScraper):
    ...
    def __init__(self, headless: bool = True, timeout: int = 30000, prefect_run_id: Optional[str] = None):
    ...
    async def discover_documents(self) -> List[DocumentMetadata]:
        ...
        try:
            ...
            while page_num < max_pages:
                ...
                load_more_button = await self.page.query_selector('#main-content > form ul button')
                if not load_more_button:
                    # Try alternative selectors
                    load_more_button = await self.page.query_selector('button:has-text("Load more")')
                ...
                while wait_count < max_wait:
                    ...
                    if new_count > current_count:
                        # New content loaded
                        self.logger.info("new_content_loaded", 
            ...
            raise ExtractionError(f"Failed to discover documents from Minnesota CARDS portal: {e}")
            ...
            for i, row in enumerate(rows):
                ...
                try:
                    ...
                    document = DocumentMetadata(
                        ...
                        filing_number=file_number.strip() if file_number.strip() else None,
                    ...
                    self.logger.warning("failed_to_process_cards_row", row_index=i, error=str(e))
                    continue
    ...
    async def _fetch_next_page(self, hx_vals: str) -> List[DocumentMetadata]:
        ...
        try:
            ...
            # Prepare form data
            form_data = {
                'documentClass': page_data.get('documentClass', 'FRANCHISE_REGISTRATIONS'),
            ...
            try:
                ...
                for i, row in enumerate(rows):
                    ...
                    try:
                        ...
                        self.logger.warning("failed_to_process_api_row", row_index=i, error=str(e))
                        continue
        ...
        try:
            ...
            for i, row in enumerate(rows):
                ...
                try:
                    ...
                    download_link = None
                    for cell in cells:
                        link = await cell.query_selector("a[href*='.pdf'], a:has-text('Download'), a:has-text('FDD')")
                        if link:
                    ...
                    self.logger.warning("failed_to_process_table_row", row_index=i, error=str(e))
                    continue
                ...
                try:
                    ...
                    if franchise_name.lower() in ['download', 'fdd', 'pdf', 'view']:
                        ...
                        if parent:
                            ...
                            for line in lines:
                                if line.lower() not in ['download', 'fdd', 'pdf', 'view'] and len(line) > 3:
                                    franchise_name = line
                                    break
    ...
    async def _extract_search_results(self) -> List[DocumentMetadata]:
        ...
        try:
            ...
            # Try list format
            list_items = await self.page.query_selector_all("li, .result, .search-result")
            for item in list_items:
                try:
                    # Look for franchise name and download link within each result
                    franchise_name_elem = await item.query_selector("h1, h2, h3, h4, .title, .name, strong")
                    if franchise_name_elem:
                        franchise_name = await franchise_name_elem.inner_text()
                        franchise_name = franchise_name.strip()
                        
                        # Look for download link
                        download_link_elem = await item.query_selector("a[href*='.pdf'], a:has-text('Download')")
    ...
    async def extract_document_metadata(self, document_url: str) -> DocumentMetadata:
        ...
        """
        try:
            self.logger.debug("extracting_minnesota_document_metadata", url=document_url)
            ...
            self.logger.error("minnesota_metadata_extraction_failed", url=document_url, error=str(e))
            raise ExtractionError(f"Failed to extract metadata from {document_url}: {e}")
    
    async def _extract_franchise_name(self) -> str:
        """Extract franchise name from current page."""
        selectors = [
            "h1", "h2", ".franchise-name", ".company-name", 
            ".title", "[data-franchise-name]", ".name"
        ]
    ...
    async def _extract_filing_date(self) -> Optional[str]:
        ...
        selectors = [
            ".filing-date", ".date", ".effective-date", 
            "[data-filing-date]", ".registration-date"
        ]
    ...
    async def _extract_filing_number(self) -> Optional[str]:
        ...
        selectors = [
            ".filing-number", ".registration-number", ".number",
            "[data-filing-number]", ".file-number"
        ]
