...
from models import (
    ...
    StateCount, Financials, FDD, FDDSection, Franchisor,
    ValidationConfig
)
...
class ValidationBypass:
    ...
    async def is_bypassed(self, entity_id: UUID, entity_type: str) -> Tuple[bool, Optional[str]]:
    ...
    async def set_bypass(self, entity_id: UUID, entity_type: str, reason: str, user_id: Optional[str] = None):
        ...
        logger.info(f"Validation bypass set for {entity_type}:{entity_id}", extra={
...
class SchemaValidator:
    ...
    async def validate_model(
        ...
        if allow_bypass:
            is_bypassed, bypass_reason = await self.bypass.is_bypassed(entity_id, entity_type)
            if is_bypassed:
        ...
        try:
            ...
            custom_errors = await self._run_custom_validators(validated_model, entity_id)
        ...
        if result.validation_duration_ms is not None:
            self._performance_metrics["total_validation_time"] += result.validation_duration_ms
    ...
    async def _validate_other_fee(self, fee: OtherFee) -> List[ValidationError]:
        ...
        if fee.amount_percentage is not None and not fee.calculation_basis:
            errors.append(ValidationError(
                ...
                severity=ValidationSeverity.ERROR,
                category=ValidationCategory.BUSINESS_RULE,
                context={"fee_name": fee.fee_name, "percentage": fee.amount_percentage}
            ))
        
        # Flag unusually high royalty percentages
        if (fee.amount_percentage is not None and 
            fee.amount_percentage > ValidationConfig.MAX_ROYALTY_PERCENTAGE and
            "royalty" in fee.fee_name.lower()):
            errors.append(ValidationError(
    ...
    async def _validate_initial_investment(self, investment: InitialInvestment) -> List[ValidationError]:
    ...
    async def _validate_fpr(self, fpr: FPR) -> List[ValidationError]:
        ...
        if fpr.sample_size is not None and fpr.sample_size < ValidationConfig.MIN_SAMPLE_SIZE_FOR_FPR:
            errors.append(ValidationError(
        ...
        if fpr.revenue_low_cents and fpr.revenue_average_cents and fpr.revenue_high_cents:
            if not (fpr.revenue_low_cents <= fpr.revenue_average_cents <= fpr.revenue_high_cents):
                errors.append(ValidationError(
        ...
        if fpr.revenue_low_cents and fpr.revenue_median_cents and fpr.revenue_high_cents:
            if not (fpr.revenue_low_cents <= fpr.revenue_median_cents <= fpr.revenue_high_cents):
                errors.append(ValidationError(
                    field_name="revenue_metrics",
        ...
        if (fpr.revenue_low_cents and fpr.revenue_high_cents and 
            fpr.revenue_average_cents and fpr.revenue_median_cents):
    ...
    async def _validate_outlet_summary(self, outlet: OutletSummary) -> List[ValidationError]:
    ...
    async def _validate_financials(self, financials: Financials) -> List[ValidationError]:
        """Custom validation for Financial Statements (Item 21)."""
        errors = []
        
        # Validate accounting equation: Assets = Liabilities + Equity
        if (financials.total_assets_cents is not None and 
            financials.total_liabilities_cents is not None and 
            financials.total_equity_cents is not None):
            
            calculated_total = financials.total_liabilities_cents + financials.total_equity_cents
        ...
        if (financials.total_equity_cents is not None and 
            financials.total_equity_cents < ValidationConfig.FLAG_NEGATIVE_EQUITY_THRESHOLD):
            errors.append(ValidationError(
            ...
            if result.errors or result.warnings or result.info:
                ...
                for error in all_errors:
                    await self.db.execute(
                        ...
                        str(error.actual_value) if error.actual_value is not None else None,
                        str(error.expected_value) if error.expected_value is not None else None,
                        error.context
                    )
    ...
    async def validate_cross_field_consistency(
        ...
        try:
            ...
            if len(item20_data) > 1:
                # Sort by fiscal year
                sorted_outlets = sorted(item20_data, key=lambda x: x.get('fiscal_year', 0))
                ...
                for i in range(1, len(sorted_outlets)):
                    ...
                    if (prev_year.get('count_end') != curr_year.get('count_start') and
                        curr_year.get('fiscal_year') == prev_year.get('fiscal_year') + 1):
                        errors.append(ValidationError(
    ...
    async def validate_data_completeness(
        ...
        try:
            ...
            populated_fields = sum(1 for key, value in data.items() 
    ...
    async def validate_temporal_consistency(
        ...
        try:
            ...
            for i in range(1, len(years)):
                ...
                numeric_fields = ['count_end', 'total_revenue_cents', 'total_assets_cents']
...
class ValidationReportGenerator:
    ...
    async def generate_fdd_report(self, fdd_id: UUID) -> ValidationReport:
        ...
        for section in sections:
            ...
            if result_row:
                ...
                validation_result = self._convert_db_to_validation_result(result_row, error_rows)
    ...
    def _convert_db_to_validation_result(self, result_row: Dict, error_rows: List[Dict]) -> ValidationResult:
    ...
    def _generate_summary(self, results: List[ValidationResult]) -> Dict[str, int]:
        ...
        summary["validation_success_rate"] = (
            (summary["valid_entities"] / max(summary["total_entities"], 1)) * 100
        )
...
async def validate_extracted_data(
    ...
    if include_completeness_check and result.is_valid:
        completeness_errors = await validator.validate_data_completeness(data, model_class)
        result.warnings.extend([e for e in completeness_errors if e.severity == ValidationSeverity.WARNING])
        result.info.extend([e for e in completeness_errors if e.severity == ValidationSeverity.INFO])
...
async def validate_fdd_sections(
    ...
    if include_cross_field_validation:
        try:
            ...
            validator = SchemaValidator(db_manager)
            cross_field_errors = await validator.validate_cross_field_consistency(fdd_id, extracted_data)
            ...
            if cross_field_errors:
                ...
                cross_field_result = ValidationResult(
                    ...
                    is_valid=not any(e.severity == ValidationSeverity.ERROR for e in cross_field_errors),
                    errors=[e for e in cross_field_errors if e.severity == ValidationSeverity.ERROR],
                    warnings=[e for e in cross_field_errors if e.severity == ValidationSeverity.WARNING],
                    info=[e for e in cross_field_errors if e.severity == ValidationSeverity.INFO]
                )
...
async def validation_performance_monitor(operation_name: str = "validation"):
    ...
    try:
        ...
        logger.info(f"{operation_name} performance", extra={
