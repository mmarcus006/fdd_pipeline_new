...
class MinerUClient:
        ...
        try:
            ...
            def run_pipeline():
                ...
                md_content = pipe.pipe_mk_markdown(
                    image_dir=os.path.basename(str(image_dir)),
                    drop_mode="none"
                )
    ...
    def _extract_layout_from_markdown(self, md_content: str) -> List[LayoutElement]:
        ...
        for line_idx, line in enumerate(lines):
            ...
            elements.append(LayoutElement(
                type=element_type,
                bbox=[50, y_position, 550, y_position + 20],  # Approximate positioning
                page=current_page,
                text=line,
    ...
    async def fallback_to_pypdf2(self, pdf_path: Path) -> DocumentLayout:
        ...
        try:
            self.logger.info(
                "Using PyPDF2 fallback for document processing",
                pdf_path=str(pdf_path)
            )
...
class FDDSectionDetector:
    ...
    def detect_sections(self, layout: DocumentLayout) -> List[SectionBoundary]:
        ...
        try:
            ...
            self.logger.info(
                ...
                sections_found=len(section_boundaries)
            )
    ...
    def _find_section_headers(self, text_elements: List[LayoutElement]) -> List[SectionBoundary]:
        """Find section headers using pattern matching."""
        boundaries = []
    ...
    def _matches_section_pattern(self, text: str, item_no: int, pattern: str) -> bool:
        ...
        if item_no == 0:
            return any(p in text_lower for p in ["cover", "introduction", "table of contents"])
    ...
    def _validate_and_fill_sections(
        ...
        for boundary in boundaries:
            if boundary.start_page > current_page:
                ...
                validated_boundaries.append(SectionBoundary(
                    item_no=0,
...
async def process_document_layout(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "MinerU processing failed, falling back to PyPDF2",
                error=str(e)
            )
