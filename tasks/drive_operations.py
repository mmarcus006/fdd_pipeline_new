...
class DriveManager:
    ...
    def folder_exists(self, name: str, parent_id: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error("Failed to check folder existence", 
                        folder_name=name, 
                        parent_id=parent_id,
                        error=str(e))
            return None
    ...
    def get_or_create_folder(self, name: str, parent_id: str) -> str:
        ...
        if folder_id:
            logger.debug("Using existing folder", 
                        folder_name=name, 
                        folder_id=folder_id)
        else:
    ...
    def download_file(self, file_id: str) -> bytes:
        ...
        try:
            ...
            logger.info("Successfully downloaded file from Google Drive", 
                       file_id=file_id,
                       file_size=len(file_content))
            return file_content
    ...
    def _get_file_path(self, file_id: str) -> str:
        ...
        try:
            ...
            logger.warning("Failed to construct file path", 
                          file_id=file_id,
                          error=str(e))
            return f"/unknown/{file_id}"
    
    def create_folder_structure(self, path: str, base_folder_id: Optional[str] = None) -> str:
    ...
    def list_files(self, folder_id: str, mime_type: Optional[str] = None) -> List[DriveFileMetadata]:
        ...
        try:
            ...
            logger.info("Listed files in folder", 
                       folder_id=folder_id,
    ...
    def upload_file_with_metadata_sync(self, 
                                      file_content: bytes, 
                                      filename: str, 
                                      folder_path: str,
                                      fdd_id: Optional[UUID] = None,
                                      document_type: str = 'original',
                                      mime_type: str = 'application/pdf') -> Tuple[str, DriveFileMetadata]:
        ...
        try:
            ...
            if existing_files:
                ...
                existing_metadata = self.get_file_metadata(existing_files[0]['drive_file_id'])
            ...
            logger.info("File uploaded and metadata synchronized", 
                       filename=filename,
                       file_id=file_id,
                       folder_path=folder_path,
                       file_size=len(file_content))
            ...
            logger.error("Failed to upload file with metadata sync", 
                        filename=filename,
                        folder_path=folder_path,
                        error=str(e))
            raise
    
    def sync_file_metadata_to_db(self, file_id: str, fdd_id: Optional[UUID] = None) -> bool:
        ...
        try:
            ...
            logger.error("Failed to sync file metadata to database", 
                        file_id=file_id,
                        error=str(e))
            return False
    ...
    def get_files_by_fdd_id(self, fdd_id: UUID) -> List[DriveFileMetadata]:
        ...
        try:
            ...
            for record in file_records:
                try:
                    ...
                    if e.resp.status == 404:
                        logger.warning("File not found in Drive, may have been deleted", 
            ...
            logger.error("Failed to get files by FDD ID", 
                        fdd_id=str(fdd_id),
                        error=str(e))
            raise
                ...
                try:
                    ...
                    if e.resp.status == 404:
                        ...
                        try:
                            ...
                            stats['errors'] += 1
                            logger.error("Failed to clean up orphaned record", 
                                       record_id=record['id'],
