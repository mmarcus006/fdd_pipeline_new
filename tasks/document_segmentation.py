...
from tasks.document_processing import DocumentLayout, SectionBoundary, FDDSectionDetector
    ...
    def split_pdf_by_pages(
        ...
        try:
            ...
            if not source_pdf_path.exists():
                raise DocumentSegmentationError(f"Source PDF not found: {source_pdf_path}")
            
            if start_page < 1 or end_page < start_page:
                raise DocumentSegmentationError(f"Invalid page range: {start_page}-{end_page}")
        ...
        try:
            ...
            for record in records:
                section = FDDSection(
                    ...
                    extracted_at=datetime.fromisoformat(record['extracted_at']) if record.get('extracted_at') else None
                )
            ...
            self.logger.error(
                "Failed to get sections by FDD ID",
                fdd_id=str(fdd_id),
                error=str(e)
            )
            raise
...
class DocumentSegmentationSystem:
        ...
        try:
            ...
            for i, section_boundary in enumerate(section_boundaries):
                ...
                try:
                    ...
                    validation_result = self.pdf_splitter.validate_pdf_content(section_pdf_bytes)
                    
                    # Generate filename
                    section_filename = f"section_{section_boundary.item_no:02d}.pdf"
                    
                    # Upload to Google Drive
                    drive_file_id, drive_metadata = self.drive_manager.upload_file_with_metadata_sync(
            ...
            self.logger.info(
                ...
                processing_time_seconds=(progress.estimated_completion - start_time).total_seconds()
            )
    ...
    def get_segmentation_status(self, fdd_id: UUID) -> Optional[Dict]:
        ...
        try:
            ...
            completed_sections = sum(1 for s in sections if s.extraction_status == ExtractionStatus.SUCCESS)
            failed_sections = sum(1 for s in sections if s.extraction_status == ExtractionStatus.FAILED)
            pending_sections = sum(1 for s in sections if s.extraction_status == ExtractionStatus.PENDING)
            ...
            self.logger.error(
                "Failed to get segmentation status",
                fdd_id=str(fdd_id),
                error=str(e)
            )
            return None
    ...
    try:
        ...
        results = {
            ...
            'processing_time_seconds': (
                progress.estimated_completion - progress.start_time
            ).total_seconds() if progress.estimated_completion else None,
            'created_sections': [
                {
                    'id': str(section.id),
        ...
        logger.error(
            "FDD document segmentation task failed",
            error=str(e)
        )
        raise
