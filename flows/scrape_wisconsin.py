...
async def scrape_wisconsin_portal(prefect_run_id: UUID) -> List[DocumentMetadata]:
    ...
    pipeline_logger = PipelineLogger(
        "scrape_wisconsin", prefect_run_id=str(prefect_run_id)
    )
    ...
    try:
        ...
        async with create_scraper(
            WisconsinScraper, prefect_run_id=prefect_run_id
        ) as scraper:
            documents = await scraper.scrape_portal()

        logger.info(
            f"Wisconsin scraping completed successfully. Found {len(documents)} documents"
        )
...
async def process_wisconsin_documents(
    ...
    logger = get_run_logger()
    pipeline_logger = PipelineLogger(
        "process_wisconsin_docs", prefect_run_id=str(prefect_run_id)
    )
    ...
    try:
        ...
        logger.info(
            f"Successfully processed {len(processed_metadata)} Wisconsin documents"
        )
        pipeline_logger.info(
        ...
        pipeline_logger.error(
            "wisconsin_processing_failed", error=str(e), run_id=str(prefect_run_id)
        )
        raise
...
async def download_wisconsin_documents(
    ...
    logger = get_run_logger()
    pipeline_logger = PipelineLogger(
        "download_wisconsin_docs", prefect_run_id=str(prefect_run_id)
    )
    ...
    try:
        ...
        logger.info(
            f"Successfully downloaded {len(downloaded_files)} Wisconsin documents"
        )
        pipeline_logger.info(
        ...
        pipeline_logger.error(
            "wisconsin_download_failed", error=str(e), run_id=str(prefect_run_id)
        )
        raise
...
async def scrape_wisconsin_flow(
    ...
    try:
        ...
        if download_documents and metadata_list:
            downloaded_files = await download_wisconsin_documents(
