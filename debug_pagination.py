...
async def debug_pagination():
    ...
    try:
        async with create_scraper(MinnesotaScraper, headless=False, timeout=60000) as scraper:
            ...
            next_page_link = await scraper.page.query_selector('a[hx-post="/api/documents/next-page"]')
            if next_page_link:
                ...
                # Get all attributes
                all_attrs = await scraper.page.evaluate('''(element) => {
                    const attrs = {};
                ...
                next_links = await scraper.page.query_selector_all('a:has-text("Next"), a:has-text("next"), a:has-text(">")')
                ...
                htmx_elements = await scraper.page.query_selector_all('[hx-post], [hx-get]')
                ...
                for i, elem in enumerate(htmx_elements):
                    ...
                    print(f"   HTMX {i+1}: <{tag}> '{text[:30]}...' hx-post='{hx_post}' hx-get='{hx_get}'")
                
                # Look for pagination container
                pagination_containers = await scraper.page.query_selector_all('.pagination, .pager, .page-nav, [class*="page"]')
            ...
            pagination_keywords = ["next-page", "pagination", "hx-post", "page-token", "pageToken"]
            ...
            result_count_elements = await scraper.page.query_selector_all('text*="results", text*="showing", text*="of"')
            ...
            for line in lines:
                if any(word in line.lower() for word in ['results', 'showing', 'page', 'of']):
