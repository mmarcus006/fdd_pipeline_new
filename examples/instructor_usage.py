...
async def example_complex_extraction_with_validation():
    ...
    print(f"Investment range: ${result.data.initial_investment_min:,} - ${result.data.initial_investment_max:,}")
    print(f"Validation errors: {result.validation_errors}")
    
    # Additional semantic validation
    validation_result = await FDDValidators.validate_business_description(
        result.data.business_description,
        result.data.franchise_name
    )
...
async def example_pdf_extraction():
    ...
    processed = await processor.process_file(
        pdf_path,
        extract_text=True,
        chunk_size=10  # 10 pages per chunk for large PDFs
    )
    ...
    if processed.gemini_file:
        ...
        print(f"Initial franchise fee: ${result.data.initial_franchise_fee_cents / 100:,.2f}")
