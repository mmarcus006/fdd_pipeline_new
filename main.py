...
@click.option('--franchise-name', help='Franchise name (will be extracted if not provided)')
...
async def health_check():
    ...
    required_tables = [
        'franchisors', 'fdds', 'fdd_sections', 'scrape_metadata',
        'item5_fees', 'item6_other_fees', 'item7_investment',
        'item19_fpr', 'item20_outlets', 'item21_financials'
    ]
    ...
    missing_tables = [t for t in required_tables if not db.health_check.check_table_exists(t)]
    if missing_tables:
...
def main():
    ...
    if len(sys.argv) > 1:
        command = sys.argv[1]
        async_commands = ['scrape', 'process-pdf', 'orchestrate', 'health-check', 'run-all']
