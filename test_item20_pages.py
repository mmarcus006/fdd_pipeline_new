...
def main():
    ...
    if page_count >= 3:
        ...
        for sec in sections[18:23]:  # Items 18-22
            ...
            print(f"  Item {sec.item_no:2d}: Pages {sec.start_page:3d}-{sec.end_page:3d} "
        ...
        for i, c in enumerate(sorted(item20_candidates, key=lambda x: x.page_number)[:5]):
            print(f"  {i+1}. Page {c.page_number}: {c.detection_method} | "
