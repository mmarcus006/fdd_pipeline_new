...
class TestWisconsinScraper:
    ...
    async def test_extract_franchise_names_from_table(
    ...
    async def test_perform_franchise_search(self, scraper):
        ...
        scraper.page.wait_for_selector = AsyncMock(
            side_effect=[search_input, search_button]
        )
    ...
    async def test_find_registered_details_link(self, scraper):
        ...
        scraper.page.evaluate = AsyncMock(
            return_value="McDonald's Corporation Registered Details"
        )
    ...
    async def test_extract_detailed_filing_info(
        self, scraper, mock_details_page_content
    ):
        """Test extraction of detailed filing information."""
        # Mock page content
        scraper.page.content = AsyncMock(return_value=mock_details_page_content)
        scraper.page.query_selector = AsyncMock(
    ...
    async def test_discover_documents_integration(self, scraper):
        ...
        scraper._extract_franchise_names_from_table = AsyncMock(
            return_value=mock_franchise_names
        )
        ...
        scraper._search_franchise_basic = AsyncMock(
            side_effect=[mock_doc1, mock_doc2, None]
        )
    ...
    async def test_extract_document_metadata_success(self, scraper):
        ...
        scraper.safe_navigate = AsyncMock()
        scraper._perform_franchise_search = AsyncMock()
        scraper._find_registered_details_link = AsyncMock(
            return_value="https://apps.dfi.wi.gov/details/12345"
        )
        scraper._extract_detailed_filing_info = AsyncMock(
            return_value={
                "filing_number": "12345",
...
class TestWisconsinScraperIntegration:
    ...
    async def test_full_workflow_mock(self):
        ...
        with (
            ...
            patch.object(
                scraper, "_extract_franchise_names_from_table"
            ) as mock_extract,
