...
class MockScraperFactory:
    ...
    def create_successful_scraper(
        ...
        mock_scraper.extract_document_metadata = AsyncMock(
            side_effect=lambda url: next(
                (doc for doc in documents if doc.source_url == url), 
                None
            )
        )
    ...
    def create_failing_scraper(
        error_message: str = "Scraping failed",
        source_name: str = "TEST"
    ) -> AsyncMock:
    ...
    def create_partial_failure_scraper(
        ...
        mock_scraper.extract_document_metadata = AsyncMock(
            side_effect=extract_effects
        )
        mock_scraper.download_file_streaming = AsyncMock(
            side_effect=download_effects
        )
...
class ScraperTestScenarios:
    ...
    def failed_discovery_scenario(
        scraper_class: type,
        error_message: str = "Portal unavailable"
    ) -> Dict[str, Any]:
