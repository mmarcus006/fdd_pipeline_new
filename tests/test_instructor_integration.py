...
class TestInstructorClient:
    ...
    async def test_extract_with_validation_errors(self, client):
        ...
        result = await client.extract_from_text(
            text="Extract test data", response_model=ComplexModel
        )
    ...
    async def test_extract_with_fallback(self, client):
        ...
        result = await client.extract_with_fallback(
            content="Test content", response_model=SimpleModel, mode=ExtractionMode.TEXT
        )
    ...
    async def test_pdf_extraction_not_implemented(self, client):
        ...
        with pytest.raises(NotImplementedError):
            await client.extract_from_pdf_file(
                pdf_path="test.pdf", response_model=SimpleModel
            )
...
class TestLLMValidation:
    ...
    def test_cross_field_validator(self):
        ...
        # Test outlet math
        assert (
            CrossFieldValidator.validate_outlet_math(
                count_start=100,
                opened=20,
                closed=5,
                transferred_in=2,
        ...
        fees = [
            ...
            {
                "fee_name": "Excessive Fee",
                "amount_cents": 150000_00,
            },  # 3x franchise fee
        ]
