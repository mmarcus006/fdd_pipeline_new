...
class TestMinnesotaScraper:
    ...
    async def scraper(self):
        ...
        async with create_scraper(
            MinnesotaScraper, headless=True, timeout=30000
        ) as scraper_instance:
    ...
    async def test_extract_cards_results_structure(self, scraper):
        ...
        assert (
            first_doc.additional_metadata.get("document_type") == "Clean FDD"
            or first_doc.document_type == "FDD"
        )
    ...
    async def test_pagination_load_more(self, scraper):
        ...
        for selector in load_more_selectors:
            try:
                ...
                if button and await button.is_visible():
                    ...
                    ), "Load more should add more results"
                    break
            except:
                continue
        ...
        if button_found:
            print(
                f"Load More button found and tested - rows increased from {initial_count} to {new_count}"
            )
        else:
    ...
    async def test_extract_document_metadata_from_detail(self, scraper):
        ...
        first_link = await scraper.page.query_selector(
            "#results tr:nth-child(2) td:nth-child(2) a"
        )
        if first_link:
    ...
    async def test_minnesota_specific_fields(self, scraper):
        ...
        if documents:
            ...
            assert doc.additional_metadata["discovery_method"] in [
                "cards_table",
                "cards_api",
            ]
