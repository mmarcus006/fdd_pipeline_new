...
class StateFlowTestBase:
    ...
    def create_test_document(
        ...
        **kwargs
        ...
        return DocumentMetadata(
            ...
            source_url=kwargs.get("source_url", f"https://test.{state_code.lower()}.gov/{document_id or 'test'}"),
            download_url=kwargs.get("download_url", f"https://test.{state_code.lower()}.gov/download/{document_id or 'test'}.pdf"),
    ...
    def create_mock_scraper(
        ...
        if should_fail:
            ...
            mock_scraper.extract_document_metadata.side_effect = Exception("Extraction failed")
        else:
    ...
    def create_mock_database(
        should_fail: bool = False,
        partial_failure_index: Optional[int] = None
    ) -> AsyncMock:
    ...
    def create_mock_drive_manager(
        ...
        if should_fail:
            ...
            mock_drive.get_or_create_folder.side_effect = Exception("Folder creation failed")
        else:
...
class StateFlowSuccessScenarios:
    ...
    async def _test_successful_discovery(
        ...
        with patch("flows.base_state_flow.create_scraper") as mock_create:
            ...
            assert all(doc.additional_metadata["source"] == state_config.state_code for doc in result)
...
class StateFlowFailureScenarios:
    ...
    async def _test_discovery_failure(
        self,
        state_config: StateConfig
    ):
