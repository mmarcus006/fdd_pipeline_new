...
class TestBaseStateFlow(StateFlowTestBase):
    ...
    @pytest.mark.asyncio
    async def test_process_state_documents_success(self, test_state_config, test_documents):
    ...
    async def test_download_state_documents_success(self, test_state_config):
        ...
        scrape_metadata = [
            self.create_test_scrape_metadata(test_doc, "TEST")
        ]
        
        mock_scraper = self.create_mock_scraper([test_doc])
        
        with patch("flows.base_state_flow.create_scraper") as mock_create, \
             patch("tasks.document_metadata.download_and_save_document") as mock_download:
    ...
    async def test_scrape_state_flow_complete(self, test_state_config, test_documents):
        ...
        with patch("flows.base_state_flow.scrape_state_portal") as mock_scrape, \
...
class TestMinnesotaFlow(StateFlowTestBase, StateFlowSuccessScenarios, StateFlowFailureScenarios):
    ...
    async def test_minnesota_csv_export(self, minnesota_documents):
        ...
        with patch("builtins.open", create=True) as mock_open:
            ...
            mock_open.assert_called_once_with(Path("test_mn.csv"), 'w', newline='', encoding='utf-8')
...
class TestWisconsinFlow(StateFlowTestBase, StateFlowSuccessScenarios, StateFlowFailureScenarios):
    ...
    async def test_wisconsin_partial_failure(self, wisconsin_documents):
        ...
        await self._test_partial_processing_failure(WISCONSIN_CONFIG, wisconsin_documents)
    ...
    async def test_wisconsin_csv_export(self, wisconsin_documents):
        ...
        with patch("builtins.open", create=True) as mock_open:
            ...
            assert result is True
            mock_open.assert_called_once_with(Path("test_wi.csv"), 'w', newline='', encoding='utf-8')
...
class TestStateFlowIntegration:
    ...
    async def test_flow_with_document_limit(self):
        ...
        all_documents = [
            StateFlowTestBase.create_test_document(f"Franchise {i}", "TEST", document_id=f"test-{i}")
            for i in range(10)
        ]
        ...
        with patch("flows.base_state_flow.scrape_state_portal") as mock_scrape, \
            ...
            mock_process.return_value = [MagicMock() for _ in range(5)]  # Process only 5
