...
class TestDriveManager:
    ...
    def drive_manager(self, mock_settings, mock_db_manager):
        ...
        with (
            patch("tasks.drive_operations.service_account"),
            patch("tasks.drive_operations.build") as mock_build,
            patch(
                "tasks.drive_operations.get_database_manager",
    ...
    def test_create_folder_success(self, drive_manager):
        ...
        # Setup
        drive_manager.service.files().create().execute.return_value = {
            "id": "new_folder_id"
        }
    ...
    def test_download_file_success(self, drive_manager):
        ...
        drive_manager.service.files().get_media.assert_called_once_with(
            fileId="file_id"
        )
    ...
    def test_get_file_metadata_success(self, drive_manager):
        ...
        drive_manager.service.files().get.return_value.execute.return_value = (
            mock_file_info
        )

        # Mock the path construction
        with patch.object(
            drive_manager, "_get_file_path", return_value="/test/path/test.pdf"
        ):
            # Execute
    ...
    def test_list_files_success(self, drive_manager):
        ...
        with patch.object(
            drive_manager,
            "_get_file_path",
            side_effect=["/path/file1.pdf", "/path/file2.pdf"],
        ):
            # Execute
    ...
    def test_verify_permissions_success(self, drive_manager):
        ...
        drive_manager.service.files().list.assert_any_call(
            q="parents in 'root_folder_id'", fields="files(id, name)", pageSize=1
        )
    ...
    def test_execute_with_retry_success_first_attempt(self, drive_manager):
        ...
        # Execute
        result = drive_manager._execute_with_retry(
    ...
    def test_upload_file_with_metadata_sync_new_file(self, drive_manager):
        ...
        with (
            patch.object(
                drive_manager, "create_folder_structure", return_value="folder_id"
            ),
            patch.object(drive_manager, "upload_file", return_value="uploaded_file_id"),
            patch.object(
                drive_manager, "get_file_metadata", return_value=mock_metadata
            ),
    ...
    def test_get_files_by_fdd_id(self, drive_manager):
        ...
        with patch.object(
            drive_manager,
            "get_file_metadata",
            side_effect=[mock_metadata_1, mock_metadata_2],
        ):
            # Execute
    ...
    def test_get_files_by_fdd_id_with_missing_file(self, drive_manager):
        ...
        with patch.object(
            drive_manager,
            "get_file_metadata",
            side_effect=[mock_metadata_1, not_found_error],
        ):
            # Execute
            result = drive_manager.get_files_by_fdd_id(fdd_id)
    ...
    def test_cleanup_orphaned_files(self, drive_manager):
        ...
        file_records = [
            ...
            {
                "id": "record2_id",
                "drive_file_id": "orphaned_file_id",
        ...
        drive_manager._db_manager.delete_record.assert_called_once_with(
            "drive_files", "record2_id"
        )
