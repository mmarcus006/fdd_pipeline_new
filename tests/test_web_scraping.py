...
class TestBaseScraper:
    ...
    async def test_initialize_failure(self, scraper):
        ...
        with patch('tasks.web_scraping.async_playwright') as mock_pw_factory:
            mock_pw_factory.return_value.start = AsyncMock(side_effect=Exception("Playwright failed"))
    ...
    async def test_retry_with_backoff_all_attempts_fail(self, scraper):
        ...
        with patch('asyncio.sleep'):
            with pytest.raises(ScrapingError, match="test_operation failed after 3 attempts"):
                await scraper.retry_with_backoff(
                    mock_operation,
                    "test_operation"
                )
    ...
    async def test_safe_navigate_with_wait_for(self, scraper):
        ...
        mock_page.wait_for_selector.assert_called_once_with("#content", timeout=scraper.timeout)
    ...
    async def test_safe_click_success(self, scraper):
        ...
        mock_page.wait_for_selector.assert_called_once_with("#button", timeout=scraper.timeout)
    ...
    async def test_safe_fill_success(self, scraper):
        ...
        mock_page.wait_for_selector.assert_called_once_with("#input", timeout=scraper.timeout)
    ...
    async def test_download_document_http_error(self, scraper):
        ...
        with pytest.raises(ScrapingError, match="document_download failed after 3 attempts"):
            await scraper.download_document("https://example.com/test.pdf")
    ...
    async def test_scrape_portal_success(self, scraper):
        ...
        scraper.extract_document_metadata_mock.side_effect = [enriched_doc1, enriched_doc2]
    ...
    async def test_scrape_portal_metadata_extraction_failure(self, scraper):
        ...
        scraper.discover_documents_mock.return_value = mock_documents
        scraper.extract_document_metadata_mock.side_effect = Exception("Metadata extraction failed")
