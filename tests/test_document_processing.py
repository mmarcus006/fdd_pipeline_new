...
class TestLayoutElement:
    ...
    def test_layout_element_optional_fields(self):
        ...
        element = LayoutElement(type="table", bbox=[0.0, 0.0, 50.0, 50.0], page=1)
...
%%EOF"""
        ...
        try:
            ...
            )  # May be 0 if PyPDF2 can't read our minimal PDF
...
class TestFDDSectionDetector:
    ...
    def test_detect_sections_empty_layout(self, detector):
        ...
        empty_layout = DocumentLayout(
            total_pages=1, elements=[], processing_time=0.0, model_version="test"
        )
    ...
    def test_validate_and_fill_sections(self, detector):
        ...
        boundaries = [
            SectionBoundary(
                item_no=1, item_name="Item 1", start_page=1, end_page=1, confidence=0.8
            ),
            SectionBoundary(
                item_no=5, item_name="Item 5", start_page=5, end_page=5, confidence=0.8
            ),
        ]
...
class TestProcessDocumentLayoutTask:
    ...
    async def test_process_document_layout_success(self):
        ...
        mock_layout = DocumentLayout(
            total_pages=5, elements=[], processing_time=10.0, model_version="test"
        )

        mock_sections = [
            SectionBoundary(
                item_no=1, item_name="Item 1", start_page=1, end_page=5, confidence=0.8
            )
        ]
