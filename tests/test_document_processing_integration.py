...
class TestExtractSectionContent:
    ...
    def mock_pdf_reader(self):
        ...
        with patch(
            "tasks.document_processing_integration.PyPDF2.PdfReader"
        ) as mock_reader:
            # Setup mock pages
            mock_page1 = Mock()
            mock_page1.extract_text.return_value = (
                "Page 10 content: Initial franchise fee"
            )
            mock_page2 = Mock()
            ...
            mock_reader_instance.pages = [Mock() for _ in range(9)] + [  # Pages 0-8
                mock_page1,
                mock_page2,
                mock_page3,
            ]  # Pages 9-11 (0-indexed)

            mock_reader.return_value = mock_reader_instance
            yield mock_reader

    @pytest.mark.asyncio
    async def test_extract_section_content_success(
        self, sample_section, mock_pdf_reader
    ):
        ...
        with patch("builtins.open", mock_open()):
            ...
            with patch(
                "tasks.document_processing_integration.FDDSectionExtractor"
                ...
                result = await extract_section_content(
                    pdf_path=pdf_path, section=sample_section, primary_model="gemini"
                )
    ...
    @pytest.mark.asyncio
    async def test_extract_section_content_extraction_error(
        self, sample_section, mock_pdf_reader
    ):
...
class TestProcessDocumentWithExtraction:
    ...
    def mock_sections(self):
        ...
        return [
            SectionBoundary(
                item_no=0, item_name="Cover", start_page=1, end_page=5, confidence=0.9
            ),
            SectionBoundary(
                item_no=5,
                item_name="Initial Fees",
    ...
    @pytest.mark.asyncio
    async def test_process_document_with_extraction_success(
        self, mock_layout, mock_sections
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "tasks.document_processing_integration.extract_section_content"
                ) as mock_extract:
                    ...
                    result = await process_document_with_extraction(
                        pdf_path=pdf_path, fdd_id=fdd_id, primary_model="gemini"
                    )
                    ...
                    assert (
                        len(result["extraction_results"]) == 3
                    )  # Only extractable sections
                    ...
                    assert (
                        "item_0" not in result["extraction_results"]
                    )  # Not extractable

    @pytest.mark.asyncio
    async def test_process_document_with_specific_sections(
        self, mock_layout, mock_sections
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    result = await process_document_with_extraction(
                        pdf_path=pdf_path, fdd_id=fdd_id
                    )
...
class TestExtractFDDSectionsBatch:
    ...
    async def test_extract_batch_with_provided_content(self, sample_sections):
        ...
        with patch(
            "tasks.document_processing_integration.extract_fdd_document"
        ) as mock_extract:
            mock_extract.return_value = {
                "fdd_id": str(fdd_id),
                "sections": {
