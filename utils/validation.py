...
from tasks.schema_validation import (
    SchemaValidator, ValidationResult, ValidationReport, 
...
class ValidationMetrics:
    ...
    def update_from_result(self, result: ValidationResult):
        ...
        if result.validation_duration_ms:
            self.avg_validation_time_ms = (
                (self.avg_validation_time_ms * (self.total_validations - 1) + 
                 result.validation_duration_ms) / self.total_validations
            )
            self.min_validation_time_ms = min(self.min_validation_time_ms, result.validation_duration_ms)
            self.max_validation_time_ms = max(self.max_validation_time_ms, result.validation_duration_ms)
        ...
        if self.total_validations > 0:
            self.success_rate = (self.successful_validations / self.total_validations) * 100
...
class ValidationPerformanceMonitor:
    ...
    def record_validation(self, result: ValidationResult):
        ...
        if (not self.metrics.peak_validation_time or 
            current_hour_count > self._hourly_counts.get(self.metrics.peak_validation_time.hour, 0)):
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "validations_per_hour": round(self.metrics.total_validations / max(runtime_hours, 0.1), 2),
            ...
            "peak_hour": self.metrics.peak_validation_time.hour if self.metrics.peak_validation_time else None
        }
    ...
    def _calculate_percentiles(self) -> Dict[str, float]:
        ...
        return {
            ...
            "p90": round(statistics.quantiles(self._validation_times, n=10)[8], 2) if len(self._validation_times) >= 10 else 0,
            "p95": round(statistics.quantiles(self._validation_times, n=20)[18], 2) if len(self._validation_times) >= 20 else 0,
            "p99": round(statistics.quantiles(self._validation_times, n=100)[98], 2) if len(self._validation_times) >= 100 else 0
        }
    ...
    def _get_top_errors(self, error_type: str) -> List[Dict[str, Any]]:
        ...
        return [
            {"name": name, "count": count, "percentage": round((count / max(self.metrics.total_validations, 1)) * 100, 2)}
            ...
            summary["hourly_trends"] = [
                {
                    ...
                    "success_rate": round((row["successful_validations"] / row["total_validations"]) * 100, 2),
...
class ValidationQualityAnalyzer:
    ...
    async def analyze_validation_quality(self, days: int = 7) -> Dict[str, Any]:
        ...
        for stat in stats:
            ...
            quality_analysis["entity_quality_scores"].append({
                ...
                "success_rate": round((stat["successful_validations"] / stat["total_validations"]) * 100, 2),
                "bypass_rate": round((stat["bypassed_validations"] / stat["total_validations"]) * 100, 2),
    ...
    def _calculate_entity_quality_score(self, stats: Dict[str, Any]) -> float:
        ...
        if avg_duration > 1000:  # > 1 second
            duration_penalty = min((avg_duration - 1000) / 100, 10)  # Max 10 point penalty
        
        final_score = max(0, base_score - bypass_penalty - error_penalty - duration_penalty)
...
async def get_validation_health_check(db_manager: DatabaseManager) -> Dict[str, Any]:
    ...
    health_check = {
        ...
        "recent_activity": {
            "total_validations_last_hour": recent_stats["total_recent"] if recent_stats else 0,
            "success_rate_last_hour": round(
                (recent_stats["successful_recent"] / max(recent_stats["total_recent"], 1)) * 100, 2
            ) if recent_stats and recent_stats["total_recent"] > 0 else 0,
            "avg_duration_ms": round(float(recent_stats["avg_duration_recent"] or 0), 2) if recent_stats else 0
        },
        "overall_quality_score": quality_analysis["overall_quality_score"],
        "high_priority_issues": len([
            r for r in quality_analysis["recommendations"] 
