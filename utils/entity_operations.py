...
class EntityResolver:
    ...
    def resolve_franchise(
        ...
        exact_match = self.db.read(
            "franchisors",
            filters={"canonical_name": franchise_name}
        )
    ...
    def deduplicate_franchises(self, batch_size: int = 100) -> Dict[str, int]:
        ...
        franchises_without_embeddings = self.db.query_builder(
            "franchisors"
        ).is_null("name_embedding").execute()
    ...
    def _merge_franchises(self, primary_id: str, duplicate_id: str):
        ...
        self.db.query_builder("fdds").update({
            "franchise_id": primary_id
        }).eq("franchise_id", duplicate_id).execute()
