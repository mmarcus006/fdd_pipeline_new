...
class MultimodalProcessor:
    ...
    async def _process_image(self, processed: ProcessedFile):
        ...
        try:
            with Image.open(file_path) as img:
                ...
                processed.extracted_text = f"Image: {width}x{height} pixels, format: {img.format}"
        except Exception as e:
            ...
            for chunk_idx in range(total_chunks):
                ...
                for page_num in range(start_page, end_page):
                    try:
                        ...
                        if text.strip():
                            chunk_text_parts.append(f"--- Page {page_num + 1} ---\n{text}")
                    except Exception as e:
                        logger.warning(f"Failed to extract text from page {page_num + 1}: {e}")
    ...
    async def process_url(
        ...
        file_name = url_path.name or f"download_{hashlib.md5(url.encode()).hexdigest()[:8]}"
