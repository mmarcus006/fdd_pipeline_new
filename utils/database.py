        ...
        if self._filters:
            ...
            for i, filter_item in enumerate(self._filters):
                ...
                if operator == '=':
                    ...
                    where_conditions.append(f"{column} BETWEEN :{param_key}_start AND :{param_key}_end")
...
class BatchOperations:
    ...
    def batch_insert_chunked(self, table_name: str, records: List[Dict], 
    ...
    def batch_upsert(self, table_name: str, records: List[Dict], 
                    conflict_columns: List[str], chunk_size: Optional[int] = None) -> int:
        ...
        try:
            ...
            for i in range(0, len(records), chunk_size):
                ...
                logger.debug("Batch upsert chunk processed", 
                           table=table_name,
                           chunk_size=len(chunk),
                           total_processed=i + len(chunk))
    ...
    def batch_update_by_ids(self, table_name: str, updates: List[Dict[str, Any]]) -> int:
    ...
    def batch_delete_by_ids(self, table_name: str, record_ids: List[Union[str, UUID]]) -> int:
...
class DatabaseManager:
    ...
    def upsert_record(self, table_name: str, record: Dict, 
        ...
        try:
            ...
            if response.data:
                logger.debug("Record upserted successfully", 
                           table=table_name,
    ...
    def get_record_by_id(self, table_name: str, record_id: Union[str, UUID]) -> Optional[Dict]:
        """Get a single record by ID."""
        try:
            client = self.get_supabase_client()
            response = client.table(table_name).select('*').eq('id', str(record_id)).execute()
    ...
    def get_records_by_filter(self, table_name: str, filters: Dict, 
        ...
        try:
            ...
            logger.error("Failed to get records by filter", 
                        table=table_name,
                        filters=filters,
                        error=str(e))
            raise
    
    def update_record(self, table_name: str, record_id: Union[str, UUID], 
                     updates: Dict) -> Optional[Dict]:
        """Update a single record by ID."""
        try:
            client = self.get_supabase_client()
            response = client.table(table_name).update(updates).eq('id', str(record_id)).execute()
    ...
    def delete_record(self, table_name: str, record_id: Union[str, UUID]) -> bool:
        ...
        try:
            client = self.get_supabase_client()
            response = client.table(table_name).delete().eq('id', str(record_id)).execute()
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(f"Query failed, retrying in {wait_time}s", 
                                 attempt=attempt + 1, 
                                 error=str(e))
    ...
    def execute_transaction_with_retry(self, operations: Callable[[Session], Any], 
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    wait_time = 2 ** attempt
                    logger.warning(f"Transaction failed, retrying in {wait_time}s", 
                                 attempt=attempt + 1, 
                                 error=str(e))
                    time.sleep(wait_time)
...
class CRUDOperations(Generic[T]):
    ...
    def get_many(self, filters: Optional[Dict] = None, 
                limit: Optional[int] = None,
