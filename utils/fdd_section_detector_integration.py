...
class HybridFDDSectionDetector:
    ...
    def detect_sections(
        ...
        **kwargs
        ...
        if (self.use_enhanced and 
            self.enhanced_detector and 
            mineru_json_path and 
            Path(mineru_json_path).exists()):
            
            try:
                logger.info(f"Attempting enhanced section detection with {mineru_json_path}")
                ...
                if self._validate_detection_results(sections, total_pages):
                    logger.info(f"Enhanced detection successful: {len(sections)} sections found")
                    return sections
                else:
        ...
        if self.fallback_to_existing and self.existing_detector and pdf_text:
            try:
                ...
                if sections:
                    logger.info(f"Existing detection successful: {len(sections)} sections found")
                    return sections
                else:
        ...
        logger.warning("All detection methods failed, creating minimal section structure")
        ...
        if total_pages:
            for section in sections:
                if section.end_page > total_pages + 5:  # Allow some tolerance
                    logger.warning(f"Section {section.item_no} end page exceeds document length")
...
def main():
    ...
    sample_json = Path("examples/2025_VALVOLINE INSTANT OIL CHANGE FRANCHISING, INC_32722-202412-04.pdf-42b85dc3-4422-4724-abf7-344b6d910da3/layout.json")
    ...
    if sample_json.exists():
        ...
        sections = detector.detect_sections(
            mineru_json_path=str(sample_json),
            total_pages=75
        )
        
        print(f"\nDetected {len(sections)} sections:")
        for i, section in enumerate(sections[:5]):
            print(f"{i+1}. Item {section.item_no}: {section.item_name[:40]}... "
