...
class FPRTable(BaseItemModel):
    ...
    table_type: Optional[str] = Field(
        None, description="Type of data (sales, costs, profits, etc.)"
    )
    ...
    period_covered: Optional[str] = Field(
        None, description="Time period covered by data"
    )
    unit_count: Optional[int] = Field(None, description="Number of units included")
    data_basis: Optional[str] = Field(
        None, description="Basis for data (actual, projected, etc.)"
    )
...
class Item19FPR(TableBasedItem):
    ...
    performance_tables: List[Dict[str, Any]] = Field(
        default_factory=list, description="Financial performance tables"
    )
    ...
    units_reporting: Optional[int] = Field(
        None, ge=0, description="Number of units included"
    )
...
class Item19FPRResponse(TableBasedItem):
    ...
    no_fpr_statement: Optional[str] = Field(
        None, description="Statement if franchisor doesn't make FPR"
    )

    # Performance tables
    performance_tables: List[FPRTable] = Field(
        default_factory=list, description="All financial performance tables"
    )
    ...
    def validate_fpr_consistency(self):
        ...
        if not self.has_fpr:
            ...
            if not self.performance_tables and not any(
                [self.average_gross_revenue_cents, self.median_gross_revenue_cents]
            ):
                self.extraction_warnings.append(
    ...
    def extract_summary_metrics(self) -> Dict[str, Any]:
        ...
        for table in self.performance_tables:
            if any(
                word in table.table_name.lower()
                for word in ["revenue", "sales", "gross"]
            ):
