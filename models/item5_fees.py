...
class InitialFeeDiscount(BaseItemModel):
    ...
    amount_cents: Optional[int] = Field(
        None, ge=0, description="Fixed discount amount in cents"
    )
    percentage: Optional[float] = Field(
        None, ge=0, le=100, description="Percentage discount"
    )
    description: str = Field(..., description="Description of the discount")
    conditions: Optional[str] = Field(
        None, description="Conditions that apply to the discount"
    )
...
class AdditionalFee(FeeBasedItem):
    ...
    refund_conditions: Optional[str] = Field(
        None, description="Conditions for refund if applicable"
    )
...
class Item5Fee(BaseItemModel):
    ...
    is_additional_unit_fee: bool = Field(
        False, description="If this is for additional units"
    )
    ...
    def validate_reasonable_amount(cls, v):
        ...
        if v > ValidationConfig.MAX_FEE_AMOUNT:
            raise ValueError(
                f"Amount exceeds reasonable maximum of ${ValidationConfig.MAX_FEE_AMOUNT / 100:,.2f}"
            )
        return v
...
class Item5FeesResponse(FeeBasedItem):
    ...
    initial_franchise_fee_cents: int = Field(
        ..., ge=0, description="Primary initial franchise fee in cents"
    )

    # Additional fees
    additional_fees: List[AdditionalFee] = Field(
        default_factory=list, description="List of additional initial fees"
    )

    # Multi-unit pricing
    additional_unit_fee_cents: Optional[int] = Field(
        None, ge=0, description="Fee for additional units/locations in cents"
    )

    # Discounts and special pricing
    discounts: List[InitialFeeDiscount] = Field(
        default_factory=list, description="Available discounts on initial fees"
    )

    # Payment terms
    payment_terms: Optional[str] = Field(None, description="Payment terms and schedule")
    due_at: FeeFrequency = Field(
        FeeFrequency.UPON_SIGNING, description="When the primary fee is due"
    )
    ...
    special_circumstances: List[str] = Field(
        default_factory=list, description="Special circumstances that affect fees"
    )
    ...
    def validate_franchise_fee(cls, v):
        ...
        if v > 100_000_000:  # $1M max
            raise ValueError(
                "Initial franchise fee exceeds reasonable maximum of $1,000,000"
            )
        return v
