...
from .base_items import (
    ...
    FeeFrequency, CalculationBasis, ValidationStatus,
    ItemValidator
)
...
class Item6OtherFee(BaseItemModel):
    ...
    fee_category: Optional[str] = Field(None, description="Category of fee (e.g., Royalty, Marketing)")
    
    # Amount (either fixed or percentage)
    amount_cents: Optional[int] = Field(None, ge=0, description="Fixed fee amount in cents")
    amount_percentage: Optional[float] = Field(None, ge=0, le=100, description="Fee as percentage")
    
    # Calculation details
    frequency: FeeFrequency = Field(..., description="How often fee is charged")
    calculation_basis: Optional[CalculationBasis] = Field(None, description="Basis for percentage fees")
...
class OtherFeeStructure(FeeBasedItem):
    ...
    fee_category: Optional[str] = Field(None, description="Category (Royalty, Marketing, etc.)")
    
    frequency: FeeFrequency = Field(..., description="Payment frequency")
    calculation_basis: Optional[CalculationBasis] = Field(None, description="How fee is calculated")
    
    minimum_cents: Optional[int] = Field(None, ge=0, description="Minimum fee if applicable")
    maximum_cents: Optional[int] = Field(None, ge=0, description="Maximum fee if applicable")
...
class Item6OtherFeesResponse(BaseItemResponse):
    ...
    royalty_frequency: FeeFrequency = Field(
        ...
        description="How often royalty is paid"
    )
    royalty_basis: CalculationBasis = Field(
        CalculationBasis.GROSS_SALES,
        description="Basis for royalty calculation"
    )
    
    # Marketing/advertising fees
    marketing_fee_percentage: Optional[float] = Field(
        None, ge=0, le=100,
        description="Marketing/advertising fee percentage"
    )
    marketing_fee_cents: Optional[int] = Field(
        None, ge=0,
        description="Fixed marketing fee if not percentage"
    )
    marketing_frequency: FeeFrequency = Field(
        FeeFrequency.MONTHLY,
        description="Marketing fee frequency"
    )
    ...
    total_fee_types: Optional[int] = Field(
        None,
        description="Total number of different fee types"
    )
    estimated_monthly_total_cents: Optional[int] = Field(
        None, ge=0,
        description="Estimated total monthly fees (if calculable)"
    )
    ...
    def calculate_totals(self):
        ...
        if self.total_fee_types is None:
            ...
            if self.royalty_percentage is not None:
                count += 1
            if self.marketing_fee_percentage is not None or self.marketing_fee_cents is not None:
                count += 1
    ...
    def validate_extraction(self) -> List[str]:
        ...
        if self.royalty_percentage is None and not any("royalty" in name for name in fee_names):
        ...
        for i, fee in enumerate(self.other_fees):
            if fee.amount_percentage and fee.amount_percentage > 20:
                issues.append(f"Fee '{fee.fee_name}' has unusually high percentage: {fee.amount_percentage}%")
