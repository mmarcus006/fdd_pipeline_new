...
class Financials(BaseItemModel):
    ...
    def validate_accounting_equations(self):
        ...
        if self.total_revenue_cents is not None:
            ...
            if (
                any(c > 0 for c in components)
                and component_sum > self.total_revenue_cents
            ):
                raise ValueError(
        ...
        if all(
            ...
            for v in [
                ...
                self.cost_of_goods_cents,
                self.gross_profit_cents,
            ]
        ):
            calculated_gross = self.total_revenue_cents - self.cost_of_goods_cents
            if (
                abs(calculated_gross - self.gross_profit_cents) > 100
            ):  # Allow $1 rounding
                raise ValueError(
        ...
        if all(
            ...
            if (
                abs(calculated_equity - self.total_equity_cents) > 100
            ):  # Allow $1 rounding
                raise ValueError(
    ...
    def operating_margin(self) -> Optional[float]:
        ...
        if self.total_revenue_cents and self.operating_income_cents:
            return round(
                (self.operating_income_cents / self.total_revenue_cents) * 100, 2
            )
        return None
    ...
    def current_ratio(self) -> Optional[float]:
        ...
        if (
            self.current_assets_cents
            and self.current_liabilities_cents
            and self.current_liabilities_cents > 0
        ):
    ...
    def debt_to_equity(self) -> Optional[float]:
        ...
        if (
            self.total_liabilities_cents
            and self.total_equity_cents
            and self.total_equity_cents > 0
        ):
...
class Item21FinancialsResponse(TableBasedItem):
    ...
    financials_data: List[Dict[str, Any]] = Field(
        default_factory=list, description="Extracted financial data by year"
    )

    # Audit information
    audit_opinions: Dict[int, Dict[str, Any]] = Field(
        default_factory=dict, description="Audit opinions by fiscal year"
    )

    # Key metrics and ratios
    key_metrics: Dict[str, Any] = Field(
        default_factory=dict, description="Key financial metrics and ratios"
    )

    # Narrative information
    going_concern_issues: Optional[str] = Field(
        None, description="Any going concern issues mentioned"
    )

    material_changes: Optional[str] = Field(
        None, description="Material changes in financial condition"
    )

    significant_events: List[str] = Field(
        default_factory=list, description="Significant events affecting financials"
    )
    ...
    def to_storage_model(self) -> List[BaseItemModel]:
        ...
        for financial_data in self.financials_data:
            ...
            financials = Financials(
                ...
                # Income statement
                total_revenue_cents=self._parse_amount(
                    financial_data.get("total_revenue")
                ),
                franchise_revenue_cents=self._parse_amount(
                    financial_data.get("franchise_revenue")
                ),
                royalty_revenue_cents=self._parse_amount(
                    financial_data.get("royalty_revenue")
                ),
                advertising_revenue_cents=self._parse_amount(
                    financial_data.get("advertising_revenue")
                ),
                product_revenue_cents=self._parse_amount(
                    financial_data.get("product_revenue")
                ),
                other_revenue_cents=self._parse_amount(
                    financial_data.get("other_revenue")
                ),
                cost_of_goods_cents=self._parse_amount(
                    financial_data.get("cost_of_goods")
                ),
                gross_profit_cents=self._parse_amount(
                    financial_data.get("gross_profit")
                ),
                operating_expenses_cents=self._parse_amount(
                    financial_data.get("operating_expenses")
                ),
                general_admin_expenses_cents=self._parse_amount(
                    financial_data.get("general_admin_expenses")
                ),
                marketing_expenses_cents=self._parse_amount(
                    financial_data.get("marketing_expenses")
                ),
                depreciation_amortization_cents=self._parse_amount(
                    financial_data.get("depreciation_amortization")
                ),
                operating_income_cents=self._parse_amount(
                    financial_data.get("operating_income")
                ),
                interest_expense_cents=self._parse_amount(
                    financial_data.get("interest_expense")
                ),
                other_income_expense_cents=self._parse_amount(
                    financial_data.get("other_income_expense")
                ),
                pretax_income_cents=self._parse_amount(
                    financial_data.get("pretax_income")
                ),
                ...
                # Balance sheet
                total_assets_cents=self._parse_amount(
                    financial_data.get("total_assets")
                ),
                current_assets_cents=self._parse_amount(
                    financial_data.get("current_assets")
                ),
                cash_equivalents_cents=self._parse_amount(
                    financial_data.get("cash_equivalents")
                ),
                accounts_receivable_cents=self._parse_amount(
                    financial_data.get("accounts_receivable")
                ),
                inventory_cents=self._parse_amount(financial_data.get("inventory")),
                fixed_assets_cents=self._parse_amount(
                    financial_data.get("fixed_assets")
                ),
                intangible_assets_cents=self._parse_amount(
                    financial_data.get("intangible_assets")
                ),
                goodwill_cents=self._parse_amount(financial_data.get("goodwill")),
                total_liabilities_cents=self._parse_amount(
                    financial_data.get("total_liabilities")
                ),
                current_liabilities_cents=self._parse_amount(
                    financial_data.get("current_liabilities")
                ),
                accounts_payable_cents=self._parse_amount(
                    financial_data.get("accounts_payable")
                ),
                accrued_expenses_cents=self._parse_amount(
                    financial_data.get("accrued_expenses")
                ),
                deferred_revenue_cents=self._parse_amount(
                    financial_data.get("deferred_revenue")
                ),
                long_term_debt_cents=self._parse_amount(
                    financial_data.get("long_term_debt")
                ),
                other_liabilities_cents=self._parse_amount(
                    financial_data.get("other_liabilities")
                ),
                total_equity_cents=self._parse_amount(
                    financial_data.get("total_equity")
                ),
                retained_earnings_cents=self._parse_amount(
                    financial_data.get("retained_earnings")
                ),
                # Cash flow
                operating_cash_flow_cents=self._parse_amount(
                    financial_data.get("operating_cash_flow")
                ),
                investing_cash_flow_cents=self._parse_amount(
                    financial_data.get("investing_cash_flow")
                ),
                financing_cash_flow_cents=self._parse_amount(
                    financial_data.get("financing_cash_flow")
                ),
                net_cash_flow_cents=self._parse_amount(
                    financial_data.get("net_cash_flow")
                ),
                # Supplementary
                number_of_outlets=financial_data.get("number_of_outlets"),
                comparable_store_sales_growth=financial_data.get(
                    "comparable_store_sales_growth"
                ),
                ebitda_cents=self._parse_amount(financial_data.get("ebitda")),
                # Audit info
                auditor_name=self.audit_opinions.get(fiscal_year, {}).get(
                    "auditor_name"
                ),
            ...
            if fiscal_year == max(
                f.get("fiscal_year", 0) for f in self.financials_data
            ):
    ...
    def _determine_statement_type(self, table: Dict[str, Any]) -> StatementType:
        ...
        if any(
            term in name or term in headers_str
            for term in ["balance sheet", "assets", "liabilities"]
        ):
            return StatementType.BALANCE_SHEET
        elif any(
            term in name or term in headers_str
            for term in ["income", "revenue", "profit loss", "p&l"]
        ):
            return StatementType.INCOME_STATEMENT
        elif any(
            term in name or term in headers_str for term in ["cash flow", "cash flows"]
        ):
            return StatementType.CASH_FLOW
        elif any(
            term in name or term in headers_str
            for term in ["equity", "shareholders", "stockholders"]
        ):
    ...
    def validate_extraction(self) -> List[str]:
        ...
        for i, financial_data in enumerate(self.financials_data):
            ...
            if not financial_data.get("total_revenue") and not financial_data.get(
                "total_assets"
            ):
    ...
    def calculate_financial_health_score(self) -> Dict[str, Any]:
        ...
        if len(self.financials_data) >= 2:
            sorted_data = sorted(
                self.financials_data, key=lambda x: x.get("fiscal_year", 0)
            )
        ...
        positive_indicators = len(
            [
                i
                for i in metrics["health_indicators"]
                if "Strong" in i or "Conservative" in i
            ]
        )
        negative_indicators = len(
            [
                i
                for i in metrics["health_indicators"]
                if "concern" in i or "loss" in i or "Declining" in i
            ]
        )
