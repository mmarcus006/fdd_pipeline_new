...
class BaseItemModel(BaseModel):
    ...
    extraction_confidence: Optional[float] = Field(
        None, ge=0, le=1, description="Confidence score for the extraction (0-1)"
    )
    ...
    raw_text: Optional[str] = Field(
        None, description="Original text this data was extracted from"
    )
...
class BaseItemResponse(BaseItemModel):
    ...
    requires_manual_review: bool = Field(
        False, description="Flag indicating if manual review is needed"
    )
...
class FeeBasedItem(BaseItemResponse):
    ...
    amount_percentage: Optional[float] = Field(
        None, ge=0, le=100, description="Fee as percentage (if applicable)"
    )
...
class InvestmentBasedItem(BaseItemResponse):
    ...
    low_cents: Optional[int] = Field(
        None, ge=0, description="Low end of range in cents"
    )
    high_cents: Optional[int] = Field(
        None, ge=0, description="High end of range in cents"
    )
...
class TableBasedItem(BaseItemResponse):
    ...
    tables: List[Dict[str, Any]] = Field(
        default_factory=list, description="Extracted tables with headers and data"
    )

    summary: Optional[str] = Field(None, description="Summary of the data presented")
...
class ItemValidator:
    ...
    def validate_percentage(
        value: float, field_name: str, max_value: float = 100.0
    ) -> Optional[str]:
