...
class OutletSummary(BaseItemModel):
    ...
    def validate_outlet_math(self):
        ...
        calculated_end = (
            self.count_start + self.opened - self.closed 
            + self.transferred_in - self.transferred_out
        )
...
class OutletTable(BaseItemModel):
    ...
    table_type: str = Field(..., description="Type of data in table (e.g., 'summary', 'by_state')")
    headers: List[str] = Field(..., description="Column headers")
    rows: List[List[Any]] = Field(..., description="Table data rows")
    fiscal_years: Optional[List[int]] = Field(None, description="Years covered in table")
...
class Item20OutletsResponse(TableBasedItem):
    ...
    outlet_summaries: List[Dict[str, Any]] = Field(
        ...
        description="Outlet data by year and type"
    )
    
    # State-by-state counts
    state_counts: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Current outlet counts by state"
    )
    
    # Company-owned outlet details
    company_owned_details: Optional[str] = Field(
        None,
        description="Details about company-owned outlets"
    )
    ...
    projected_openings: Optional[Dict[str, int]] = Field(
        None,
        description="Projected new outlet openings by year"
    )
    ...
    def to_storage_model(self) -> List[BaseItemModel]:
        ...
        for summary_data in self.outlet_summaries:
            summary = OutletSummary(
                ...
                outlet_type=OutletType(summary_data.get("outlet_type", "").lower().replace("-", "_").replace(" ", "_")),
    ...
    def validate_extraction(self) -> List[str]:
            ...
            for summary in self.outlet_summaries:
                ...
                if year and outlet_type:
                    ...
                    if key in years_seen:
                        issues.append(f"Duplicate outlet summary for {year} {outlet_type}")
                    years_seen.add(key)
        ...
        if self.state_counts and self.outlet_summaries:
            ...
            recent_year = max(
                s.get("fiscal_year", 0) for s in self.outlet_summaries
            )
            recent_summaries = [
                s for s in self.outlet_summaries 
                if s.get("fiscal_year") == recent_year
            ]
            
            outlet_franchised = sum(
                s.get("count_end", 0) for s in recent_summaries
        ...
        if (self.multi_unit_operators is not None and 
            self.total_franchisees is not None and
            self.multi_unit_operators > self.total_franchisees):
            issues.append(
                "Multi-unit operators cannot exceed total franchisees"
            )
            ...
            if len(franchised_by_year) >= 2:
                ...
                if franchised_by_year[first_year] > 0:
                    ...
                    metrics["franchised_cagr"] = round(
                        (pow(franchised_by_year[last_year] / franchised_by_year[first_year], 
        ...
        if self.state_counts:
            ...
            if total_outlets > 0:
                ...
                sorted_states = sorted(
                    self.state_counts,
                    key=lambda x: x.get("franchised_count", 0) + x.get("company_owned_count", 0),
                    reverse=True
                )[:5]
                ...
                metrics["states_with_presence"] = len([
                    s for s in self.state_counts
                    if s.get("franchised_count", 0) + s.get("company_owned_count", 0) > 0
                ])
