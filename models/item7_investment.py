...
from .base_items import (
    BaseItemModel, BaseItemResponse, InvestmentBasedItem,
    ValidationStatus, ItemValidator, ItemCollection
)
...
class Item7Investment(InvestmentBasedItem):
    ...
    standardized_category: Optional[str] = Field(None, description="Standardized category name")
    ...
    is_refundable: Optional[bool] = Field(None, description="If investment is refundable")
...
class Item7InvestmentResponse(BaseItemResponse):
    ...
    total_investment_low_cents: Optional[int] = Field(
        None, ge=0,
        description="Total minimum investment"
    )
    total_investment_high_cents: Optional[int] = Field(
        None, ge=0,
        description="Total maximum investment"
    )
    
    # Additional information
    financing_available: Optional[bool] = Field(
        None,
        description="Whether franchisor offers financing"
    )
    financing_details: Optional[str] = Field(
        None,
        description="Details about available financing"
    )
    
    # Special notes
    includes_working_capital: bool = Field(
        True,
        description="Whether totals include working capital"
    )
    real_estate_included: bool = Field(
        False,
        description="Whether real estate purchase is included"
    )
    ...
    def calculate_totals(self):
        ...
        if self.total_investment_low_cents is None:
            total_low = sum(
                item.low_cents for item in self.investment_items 
        ...
        if self.total_investment_high_cents is None:
            total_high = sum(
                item.high_cents for item in self.investment_items
    ...
    def validate_totals_match(self):
        ...
        actual_low = sum(
            item.low_cents or 0 for item in self.investment_items
        )
        actual_high = sum(
            item.high_cents or 0 for item in self.investment_items
        )
    ...
    def get_items_by_category(self, category: str) -> List[InvestmentItem]:
        ...
        return [
            item for item in self.investment_items
    ...
    def validate_extraction(self) -> List[str]:
        ...
        categories = [InvestmentCategory.standardize(item.category) for item in self.investment_items]
        ...
        for i, item in enumerate(self.investment_items):
            ...
            if item.high_cents and item.high_cents > ValidationConfig.MAX_INVESTMENT_AMOUNT:
                issues.append(
...
class InvestmentSummary(BaseItemModel):
    ...
    def validate_totals(cls, v):
        ...
        if v > ValidationConfig.MAX_INVESTMENT_AMOUNT:
            raise ValueError(f"Total exceeds reasonable maximum of ${ValidationConfig.MAX_INVESTMENT_AMOUNT / 100:,.2f}")
        return v
