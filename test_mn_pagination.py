...
async def test_pagination():
    ...
    try:
        ...
        async with create_scraper(MinnesotaScraper, headless=False, timeout=60000) as scraper:
            ...
            print(f"\n📋 SAMPLE DOCUMENTS:")
            sample_indices = [0, 499, 999, len(documents)-1] if len(documents) > 1000 else [0, len(documents)//2, len(documents)-1]
            ...
            for i, idx in enumerate(sample_indices):
                if idx < len(documents):
                    ...
                    print(f"      Discovery method: {doc.additional_metadata.get('discovery_method', 'N/A')}")
            ...
            for doc in documents:
                results.append({
                    ...
                    "discovery_method": doc.additional_metadata.get('discovery_method')
                })
            ...
            if len(documents) > 500:
                print(f"🎯 SUCCESS: Pagination worked! Got {len(documents)} documents (more than 500)")
                
                # Check if we have documents from different discovery methods
                methods = set(doc.additional_metadata.get('discovery_method') for doc in documents)
                print(f"📊 Discovery methods used: {methods}")
                
                if 'cards_api' in methods:
                    print("✅ API pagination method was used successfully")
                else:
                    print("⚠️  Only table method was used - API pagination may not have worked")
            else:
                print(f"⚠️  Only got {len(documents)} documents - pagination may not be working")
